...
*/
bool Mind_Final::CheckNumber(const std::string& value){
...
*/
void Mind_Final::GetSelectImages(const std::string& value){
    //if value is "", no select images
    if(value == ""){
        HIAI_ENGINE_LOG(HIAI_IDE_WARNING, "[Mind_Final] select images info is null!");
        return;
    }
    std::string token;
    ...
    while (std::getline(tokenStream, token, ','))
    {
        if(CheckNumber(token)){
            ...
            if(frameId >= 0 && frameId < dataset_info_.size()){
                ...
                HIAI_ENGINE_LOG(HIAI_IDE_WARNING, "[Mind_Final] select image frame id must be in [0, %u), but the input is %u !", dataset_info_.size(), frameId);
            }
        }else{
            HIAI_ENGINE_LOG(HIAI_IDE_WARNING, "[Mind_Final] select image info must be positive integer, but the input is '%s' !", token.c_str());
        }
    }
    if(select_images_.size() > 0){
        HIAI_ENGINE_LOG(HIAI_IDE_INFO, "[Mind_Final] select %u images success!", select_images_.size());
    }
}
...
*/
void Mind_Final::GetRandomImages(){
    ...
    HIAI_ENGINE_LOG(HIAI_IDE_INFO, "[Mind_Final] select random %u images success!", data_config_->randomNumber);
}
...
* @[return]: HIAI_StatusT
*/
HIAI_StatusT Mind_Final::MakeDatasetInfo(){
...
*/
void Mind_Final::GetSelectOrRandomImages(const std::string& selectImages, const std::string& randomNumber){
    //clear vector
    ...
    if(data_config_->runMode == "random"){
        if(CheckNumber(randomNumber)){
            ...
            if(randomNum > 0 && randomNum < dataset_info_.size()){
                ...
                HIAI_ENGINE_LOG(HIAI_IDE_WARNING, "[Mind_Final] random number must be in (0, %u), but the input is %u !", dataset_info_.size(), randomNum);
            }
        }else{
            HIAI_ENGINE_LOG(HIAI_IDE_WARNING, "[Mind_Final] random number must be positive integer, but the input is '%s' !", randomNumber.c_str());
        }
    ...
    if(select_images_.size() == 0){
        HIAI_ENGINE_LOG(HIAI_IDE_WARNING, "[Mind_Final] There is no random or select images, run all images!");
...
*/
void Mind_Final::GetBatchSize(const std::string& value){
    bool check = false;
    ...
    if(!check){
        HIAI_ENGINE_LOG(HIAI_IDE_WARNING, "[Mind_Final] batch size must be positive integer, but the input is '%s'. Use default batch size %u !", value.c_str(), data_config_->batchSize);
    }
}

HIAI_StatusT Mind_Final::Init(const hiai::AIConfig& config, const  std::vector<hiai::AIModelDescription>& model_desc)
{
    HIAI_ENGINE_LOG(HIAI_IDE_INFO, "[Mind_Final] start init!");

    data_config_ = std::make_shared<DataConfig>();
    if(data_config_ == NULL){
        HIAI_ENGINE_LOG(HIAI_IDE_ERROR, "[Mind_Final] make shared for DataConfig error!");
        return HIAI_ERROR;
    }
    ...
    if(realpath(data_config_->path.c_str(), path) == NULL){
        HIAI_ENGINE_LOG(HIAI_IDE_ERROR, "[Mind_Final] can not find path!");
        return HIAI_ERROR;
    }
    ...
    HIAI_ENGINE_LOG(HIAI_IDE_INFO, "[Mind_Final] end init!");
    return ret;
}
...
*/
bool Mind_Final::GetImageBuffer(const char* path, uint8_t* imageBufferPtr, uint32_t imageBufferLen, uint32_t frameId){
    bool ret = false;
    FILE * file = fopen64(path, "r");
    if (NULL == file)
    {
        HIAI_ENGINE_LOG(HIAI_IDE_ERROR, "[Mind_Final] Error: open file %s failed", path);
        return ret;
    }
    do
    {
        ...
        {
            HIAI_ENGINE_LOG(HIAI_IDE_ERROR, "[Mind_Final] fseeko64 offset = %u failed in GetImageBuffer", frameId * imageBufferLen);
            break;
        }
        if (imageBufferLen != fread(imageBufferPtr, 1, imageBufferLen, file))
        {
            HIAI_ENGINE_LOG(HIAI_IDE_ERROR, "[Mind_Final] fread length = %u failed in GetImageBuffer", imageBufferLen);
            break;
        }
        ret = true;
    } while (0);
...
*/
shared_ptr<EvbImageInfo> Mind_Final::makeEvbImageInfo(int index){
    ...
    if(evbImageInfo == NULL){
        HIAI_ENGINE_LOG(HIAI_IDE_ERROR, "[Mind_Final] make shared for EvbImageInfo error!");
        return NULL;
    }
    ...
    if(HIAI_OK != get_ret || NULL == imageBufferPtr){
        HIAI_ENGINE_LOG(HIAI_IDE_ERROR, "[Mind_Final] DMalloc buffer error!");
        return NULL;
    }
    ...
    if(!ret){
        ...
        if(HIAI_OK != get_ret){
            HIAI_ENGINE_LOG(HIAI_IDE_ERROR, "[Mind_Final] DFree buffer error!");
        }
        imageBufferPtr = NULL;
        return NULL;
    }
...
* @[return]: HIAI_StatusT
*/
HIAI_StatusT Mind_Final::makeImageInfo(NewImageParaT* imgData, int index) {
    ...
    if(imageBufferPtr == NULL){
        HIAI_ENGINE_LOG(HIAI_IDE_ERROR, "[Mind_Final] alloc buffer error in makeImageInfo");
        return HIAI_ERROR;
    }
...
* @[return]: HIAI_StatusT
*/
HIAI_StatusT Mind_Final::SendBatch(int batchId, int batchNum, std::shared_ptr<BatchImageParaWithScaleT> imageInfoBatch){
    ...
    do{
        ...
        {
            HIAI_ENGINE_LOG(HIAI_IDE_INFO, "[Mind_Final] queue full, sleep 200ms");
            usleep(SEND_DATA_INTERVAL_MS);
        }
    }while(hiai_ret == HIAI_QUEUE_FULL);

    if(HIAI_OK != hiai_ret){
        HIAI_ENGINE_LOG(HIAI_IDE_ERROR, "[Mind_Final] SendData batch %u failed! error code: %u", batchId, hiai_ret);
    }
    return hiai_ret;
}
...
* @[return]: HIAI_StatusT
*/
HIAI_StatusT Mind_Final::SendEvbBatch(int batchId, int batchNum, std::vector<std::shared_ptr<EvbImageInfo>>& evbImageInfoBatch){
    HIAI_StatusT hiai_ret = HIAI_OK;
    for(unsigned int i = 0; i < evbImageInfoBatch.size(); ++i){
        ...
        do{
            ...
            {
                HIAI_ENGINE_LOG(HIAI_IDE_INFO, "[Mind_Final] hiai graph no useful memory, sleep 200ms");
                usleep(SEND_DATA_INTERVAL_MS);
            }
        }while(HIAI_GRAPH_NO_USEFUL_MEMORY == hiai_ret  || HIAI_GRAPH_INVALID_VALUE == hiai_ret);

        if(HIAI_OK != hiai_ret){
            HIAI_ENGINE_LOG(HIAI_IDE_ERROR, "[Mind_Final] SendData image frame id %u failed! error code: %u", evbImageInfoBatch[i]->frame_ID, hiai_ret);
...
*/
int Mind_Final::GetFrameId(int index){
    int frameId = index;
...
* free evb buffer
**/
void Mind_Final::FreeEvbBuffer(const std::vector<std::shared_ptr<EvbImageInfo>>& evbImageInfoBatch, int start){
    HIAI_StatusT ret = HIAI_OK;
    for(unsigned int i = start; i < evbImageInfoBatch.size(); ++i){
        if(evbImageInfoBatch[i]->pucImageData != NULL){
            ...
            if(HIAI_OK != ret){
                HIAI_ENGINE_LOG(HIAI_IDE_ERROR, "[Mind_Final] free buffer error!");
            }
...
* @[return]: HIAI_StatusT
*/
HIAI_StatusT Mind_Final::RunOnSameSide(){
    HIAI_StatusT ret = HIAI_OK;
    ...
    int i = 0;
    HIAI_ENGINE_LOG(HIAI_IDE_INFO, "[Mind_Final] run on %s, run %u images, batch size is %u", data_config_->target.c_str(), totalCount, data_config_->batchSize);
    for(int batchId = 0; batchId < batchNum && i < totalCount; batchId++){
        ...
        if(imageInfoBatch == NULL){
            HIAI_ENGINE_LOG(HIAI_IDE_ERROR, "[Mind_Final] make shared for BatchImageParaWithScaleT error!");
            return HIAI_ERROR;
        }
        for(int j = 0 ; j < data_config_->batchSize && i < totalCount; j++){
            ...
            if(HIAI_OK != ret) {
                HIAI_ENGINE_LOG(HIAI_IDE_ERROR, "[Mind_Final] Error: make image info frame id %u for batch id %u failed! Stop to send images!",frameId, batchId);
                return ret;
            }
...
* @[return]: HIAI_StatusT
*/
HIAI_StatusT Mind_Final::RunOnDifferentSide(){
    HIAI_StatusT ret = HIAI_OK;
    int totalCount = GetTotalCount();
    int batchNum = totalCount % data_config_->batchSize == 0 ? totalCount / data_config_->batchSize : totalCount / data_config_->batchSize + 1;
    int i = 0;
...
*/
HIAI_StatusT Mind_Final::SendSentinelImage()
{
    HIAI_StatusT ret = HIAI_OK;
...
*/
HIAI_IMPL_ENGINE_PROCESS("Mind_Final", Mind_Final, INPUT_SIZE)
{
    HIAI_ENGINE_LOG(HIAI_IDE_INFO, "[Mind_Final] start process!");
    ...
    //send sentinel image
    HIAI_ENGINE_LOG(HIAI_IDE_INFO, "[Mind_Final] send sentinel image!");
    ret = SendSentinelImage();
    HIAI_ENGINE_LOG(HIAI_IDE_INFO, "[Mind_Final] end process!");
    return ret;
}
